/*
题意：坑爹的题意， 有不同的一片片农田， 每一片农田里的所有农场都是联通的。 农田的半径是任意两个农场之间的最大距离值。
     你可以在两个不相通的农田之间连一条边，使得这个半径最小。
思路： 把每一个农场看作是一个单体,然后计算出它在的农田中的以它为一个定点的最长距离。 怎么区分联通？ 就是看这两个农场之间的距离是不是INF，
      如果是我们就可以尝试着连一条边。
*/
